...
const createGroup = async (req, res) => {
    try {
        ...
        const memGrp = await GroupMembers.create({ groupId: newGrp._id, userId, isAdmin: true })
...
const updateGroup = async (req, res) => {
    try {
        ...
        const updatedGrp = await Groups.findByIdAndUpdate(groupId, updatedDetails, { new: true });
        if (!updatedGrp) {
            return res.status(404).json({ message: "Group not found" });
        }
...
const addMember = async (req, res) => {
    try {
        ...
        const alreadyExists = await GroupMembers.findOne({ userId: memberToAdd, groupId });
        if (alreadyExists) {
            return res.status(400).json({ message: "User is already a member of this group." });
        }

        const newMember = await GroupMembers.create({ groupId, userId: memberToAdd })
...
const removeMember = async (req, res) => {
    try {
        ...
        const removedMem = await GroupMembers.deleteOne({ groupId, userId: memberToRemove })
